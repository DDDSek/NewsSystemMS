namespace NewsSystem.Infrastructure.ArticleCreation.Repositories
{
    ...
    internal class ArticleRepository : DataRepository<IArticleCreationDbContext, Article>, IArticleRepository
    {
        ...
        public async Task<Article> FindArticleComment(int id, CancellationToken cancellationToken = default)
            => await this
                .All()
                .Where(c => c.Comments.Any(c => c.Id == id))
                .SingleOrDefaultAsync(c => c.Id == id, cancellationToken);

        public async Task<Comment> FindComment(int id, CancellationToken cancellationToken = default)
            => await this.Data.Comments.FirstOrDefaultAsync(c => c.Id == id, cancellationToken);
